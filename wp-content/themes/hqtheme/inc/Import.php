<?php
if (!class_exists('HQTheme_Import')) {

    /**
     * Import demo content & setting
     * 
     * @link 
     * @since HQTheme 1.0
     */
    class HQTheme_Import {

        public function __construct() {
            return;
            $HQ_Options_Importer = new HQ_Options_Importer();

            return;
            $HQ_Import = new HQ_Import();
            $HQ_Import->import(get_template_directory() . '/demo-data/educatingwithcolor/data.xml');
        }
...
if (!class_exists('HQ_Import')) {
    ...
    if (class_exists('WP_Importer')) {
        ...
        class HQ_Import extends WP_Importer {
             ...
             */
            function parse($file) {
                $parser = new HQ_Parser_SimpleXML();
    ...
    } // class_exists( 'HQ_Import' )
}
if (!class_exists('HQ_Parser_SimpleXML')) {

    class HQ_Parser_SimpleXML {
...
// -----------------------------------------
if (!class_exists('HQ_Options_Importer')) {

    class HQ_Options_Importer {
        ...
        public function __construct() {
            $file_contents = file_get_contents(get_template_directory() . '/demo-data/educatingwithcolor/settions.json');
            $this->import_data = json_decode($file_contents, true);
            $this->import();
        }

        public function __clone() {
            wp_die("Please don't __clone HQ_Options_Importer");
        }

        public function __wakeup() {
            wp_die("Please don't __wakeup HQ_Options_Importer");
        }

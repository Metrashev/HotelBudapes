 ...
 */
function hq_hex_shift($supplied_hex, $shift_method, $percentage = 50) {
...
function hq_hex2rgb($colour, $delimiter = '-') {
    if ($colour[0] == '#') {
 ...
 */
// function hq_apply_formatting() {
//  // Enable shortcodes in text widgets
//  add_filter( 'widget_text', 'do_shortcode' );
//  // Enable shortcodes in category descriptions
//  add_filter( 'category_description', 'do_shortcode' );
//  // Enable auto-formatting
//  if ( get_option( 'hq_option_custom-formatting' ) === 'on' ) {
//   // Disable WordPress native content formatters
//   remove_filter( 'the_content', 'wpautop' );
//   remove_filter( 'the_content', 'wptexturize' );
//   // Apply custom formatter function
//   add_filter( 'the_content', 'hq_custom_formatter', 99 );
//   add_filter( 'widget_text', 'hq_custom_formatter', 99 );
//   add_filter( 'category_description', 'hq_custom_formatter', 99 );
//  }
//  // Fix for large posts, http://core.trac.wordpress.org/ticket/8553
//  @ini_set( 'pcre.backtrack_limit', 500000 );
// }
// add_action( 'init', 'hq_apply_formatting' );
 ...
 */
// function hq_custom_formatter( $content ) {
//  // Prepare variables
//  $new_content = '';
...
function hq_apply_formatting() {
    ...
    if (get_option('hq_option_custom-formatting') === 'on') {
        // Apply custom formatter function
        add_filter('the_content', 'hq_clean_shortcodes');
    }
}
 ...
 * @return string Formatted content with clean shortcodes content
 */
function hq_clean_shortcodes($content) {
    $p = hq_cmpt();
    $array = array(
        '<p>[' => '[',
        ']</p>' => ']',
 ...
 */
function hq_do_shortcode($content, $pre) {
 ...
 */
function hq_compatibility_mode_prefix() {
    return get_option('hq_option_prefix');
}

/**
 * Shortcut for hq_compatibility_mode_prefix()
 */
function hq_cmpt() {
    return hq_compatibility_mode_prefix();
}
 ...
 */
function hq_image_resize($url, $width = NULL, $height = NULL, $crop = true, $retina = false) {
    global $wp_version;
 ...
 */
function hq_delete_resized_images($post_id) {
...
add_action('delete_attachment', 'hq_delete_resized_images');

class HQ_Tools {

    function __construct() {
        add_action('wp_ajax_hq_example_preview', array(__CLASS__, 'example'));
    ...
    public static function reset_users_cache() {
        if (( isset($_GET['update']) || isset($_GET['updated'])))
            if ($_GET['update'] === 'del' || $_GET['update'] === 'add' || $_GET['updated'] === '1')
                delete_transient('su/users_cache');
    }
    ...
    public static function get_slides($args) {
        ...
        if ($args['gallery'] !== null || ( $args['source'] === 'none' && get_option('hq_option_galleries-432') ))
        ...
        if (is_array($query->posts))
            foreach ($query->posts as $post) {
                ...
                elseif ($args['link'] === 'custom')
                    $slide['link'] = get_post_meta($post->ID, 'hq_slide_link', true);
    ...
    public static function get_slides_432($args) {
        ...
        $galleries = get_option('hq_option_galleries-432');
        // No galleries found
    ...
    public static function example() {
        ...
        // Cache not found
            ...
            // Check for code
            if (!$code)
                die('<p class="hq-examples-error">' . __('Example code does not found, please check it later', 'su') . '</p>');
            // Clean-up the code
            $code = str_replace(array("\t", '%hq_'), array('  ', hq_cmpt()), $code);
            // Split code
            $chunks = explode('-----', $code);
            // Show snippets
            ...
            foreach ($chunks as $chunk) {
                ...
                echo '<div class="hq-examples-code"><span class="hq-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __('Get the code', 'su') . '</span><textarea rows="' . $rows . '">' . esc_textarea($chunk) . '</textarea></div>';
            }
    ...
    public static function reset_examples() {
        foreach ((array) HQ_Data::examples() as $example)
    ...
    public static function icons() {
        $icons = array();
        if (is_callable(array('HQ_Data', 'icons')))
            foreach ((array) HQ_Data::icons() as $icon) {
    ...
    public static function slide_link_input($form_fields, $post) {
        $form_fields['hq_slide_link'] = array(
            'label' => __('Slide link', 'su'),
            'input' => 'text',
            'value' => get_post_meta($post->ID, 'hq_slide_link', true),
    ...
    public static function slide_link_save($post, $attachment) {
        if (isset($attachment['hq_slide_link']))
            update_post_meta($post['ID'], 'hq_slide_link', $attachment['hq_slide_link']);
        return $post;
    }
    ...
    public static function error($prefix = false, $message = false) {
        if (!$prefix && !$message)
            return '';
        $return = array('<div class="hq-error" style="padding:10px;border:1px solid #f03;color:#903;background:#fde">');
        if ($prefix)
            $return[] = '<strong>' . $prefix . '</strong><br/>';
        $return[] = $message;
        $return[] = '</div>';
...
/**
 * Shortcut for HQ_Tools::decode_shortcode()
 */
function hq_scattr($value) {
    return HQ_Tools::do_attr($value);
}

/**
 * Shortcut for HQ_Tools::get_icon()
 */
function hq_get_icon($args) {
    return HQ_Tools::get_icon($args);
}

...
?>
        <div class="otw-row otw-sc-portfolio" <?php echo $style_width; ?>>
              ...
              <?php $taxo = get_object_taxonomies( 'cool-simple-portfolio' );
                foreach ( $taxo as $tax_name ) {
                    $categories = get_categories('taxonomy='.$tax_name);
                    $i = 0; $len = count( $categories );
                    foreach ($categories as $category) {
                        if ($i == 0) { ?><ul class="cool-simple-portfolio-filter"><?php }
              ...
              ?>
                  ...
                  <?php if (is_page()) { $paged = (get_query_var('paged')) ? get_query_var('paged') : 1; query_posts('post_type=cool-simple-portfolio&paged='.$paged); } ?>
                  <?php if (have_posts()): while (have_posts()) : the_post(); ?>
                      <li data-type="<?php foreach(get_the_terms($post->ID, 'cool-simple-portfolio-category') as $term) echo $term->slug.' ' ?>" data-id="id-<?php echo($post->post_name) ?>">
